...
label monika_sing_song_pool_menu:
    python:
        ...
        switch = (_("I'd like to hear a [switch_str] song instead", "monika_sing_song_pool_menu"), False, False, 20)
        ...
        renpy.say(m, _("[which] song would you like me to sing?[end]"), interact=False)
...
label monika_sing_song_analysis:
    ...
    show monika 1eua at t21
    $ renpy.say(m, _("[which] song would you like to talk about?"), interact=False)
...
label mas_sing_song_rerandom:
    ...
    call mas_rerandom from _call_mas_rerandom_1
    return _return
...
label mas_song_when_you_say_nothing_at_all_long:
    ...
    call mas_song_when_you_say_nothing_at_all (from_long=True) from _call_mas_song_when_you_say_nothing_at_all
    ...
    call mas_song_when_you_say_nothing_at_all (from_long=True) from _call_mas_song_when_you_say_nothing_at_all_1
...
label mas_song_strawberry_long:
    call mas_song_strawberry (from_long=True) from _call_mas_song_strawberry
...
label mas_song_save_the_last_dance_for_me:
    call mas_song_save_the_last_dance_for_me_lyrics from _call_mas_song_save_the_last_dance_for_me_lyrics
    m 6dublu "..."
    ...
    if not mas_getEVL_shown_count("mas_song_save_the_last_dance_for_me"):
        ...
        menu:
            ...
            "Yes.":
                ...
                call mas_song_save_the_last_dance_for_me_analysis (from_song=True) from _call_mas_song_save_the_last_dance_for_me_analysis
            "No.":
...
label mas_song_save_the_last_dance_for_me_analysis(from_song=False):
    if not from_song:
        call mas_song_save_the_last_dance_for_me_lyrics from _call_mas_song_save_the_last_dance_for_me_lyrics_1
    else:
...
label mas_monika_plays_or(skip_leadin=False):
    ...
    window hide
    call mas_timed_text_events_prep from _call_mas_timed_text_events_prep_3
    $ mas_temp_zoom_level = store.mas_sprites.zoom_level
    ...
    call mas_timed_text_events_wrapup from _call_mas_timed_text_events_wrapup_3
    window auto
