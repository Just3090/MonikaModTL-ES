...
init -3 python in mas_piano_keys:
    ...
    MISS_KEY = _("key '{0}' is missing.")
    NOTE_BAD = _("bad note list.")
    PNOTE_BAD = _("bad post note list.")
    EXP_BAD = _("expression '{0}' is invalid.")
    EVT_BAD = _("ev timeout '{0}' is invalid.")
    VIST_BAD = _("vis timeout '{0}' is invalid.")
    VERSE_BAD = _("verse '{0}' is invalid.")
    PTEXT_BAD = _("bad posttext value.")
    EXTRA_BAD = _("extra key '{0}' found.")

    NOTES_BAD = _("pnm list cannot be empty.")
    VERSES_BAD = _("verse list cannot be empty.")
    LABEL_BAD = _("label '{0}' does not exist.")
    WAIT_BAD = _("wait time '{0}' is invalid.")
    L_VERSE_BAD = _("verse '{0}' out of bounds.")

    PNM_LOAD_TRY = _("Loading PNM '{0}'...")
    PNM_LOAD_SUCC = _("PNM '{0}' loaded successfully!")
    PNM_LOAD_FAILED = _("PNM '{0}' load failed.")
...
init 810 python:
    ...
    class PianoDisplayable(renpy.Displayable):
        ...
        def render(self, width, height, st, at):
            ...
            # if renpy.android:
            #     for key, (img, x, y) in self.overlays.items():
            #         # Crea un botón invisible sobre cada tecla
            #         r.interact(
            #             area=(
            #                 self.ZZPK_IMG_BACK_X + x,
            #                 self.ZZPK_IMG_BACK_Y + y,
            #                 self.ZZPK_IMG_IKEY_WIDTH if img in [Image(self.ZZPK_W_OVL_LEFT), Image(self.ZZPK_W_OVL_CENTER), Image(self.ZZPK_W_OVL_RIGHT), Image(self.ZZPK_W_OVL_PLAIN)] else self.ZZPK_IMG_EKEY_WIDTH,
            #                 self.ZZPK_IMG_IKEY_HEIGHT if img in [Image(self.ZZPK_W_OVL_LEFT), Image(self.ZZPK_W_OVL_CENTER), Image(self.ZZPK_W_OVL_RIGHT), Image(self.ZZPK_W_OVL_PLAIN)] else self.ZZPK_IMG_EKEY_HEIGHT
            #             ),
            #             clicked=renpy.curry(self._on_touch_key)(key)
            #         )
        ...
        def _on_touch_key(self, key):
            # Simula la pulsación de la tecla correspondiente
            if not self.pressed.get(key, True):
                self.pressed[key] = True
                self.note_hit = True
                self.played.append(key)
                # Ejecuta la lógica de juego según el estado
                ...
                renpy.redraw(self, 0)
            # Simula soltar la tecla después de un breve tiempo
            renpy.timeout(0.1)
        ...
        def event(self, ev, x, y, st):
            ...
            if renpy.android and ev.type == pygame.MOUSEBUTTONDOWN:
                for key, (img, key_x, key_y) in self.overlays.items():
                    area_x = self.ZZPK_IMG_BACK_X + key_x
                    area_y = self.ZZPK_IMG_BACK_Y + key_y
                    area_w = self.ZZPK_IMG_IKEY_WIDTH if img in [Image(self.ZZPK_W_OVL_LEFT), Image(self.ZZPK_W_OVL_CENTER), Image(self.ZZPK_W_OVL_RIGHT), Image(self.ZZPK_W_OVL_PLAIN)] else self.ZZPK_IMG_EKEY_WIDTH
                    area_h = self.ZZPK_IMG_IKEY_HEIGHT if img in [Image(self.ZZPK_W_OVL_LEFT), Image(self.ZZPK_W_OVL_CENTER), Image(self.ZZPK_W_OVL_RIGHT), Image(self.ZZPK_W_OVL_PLAIN)] else self.ZZPK_IMG_EKEY_HEIGHT
                    if area_x <= x <= area_x + area_w and area_y <= y <= area_y + area_h:
                        self._on_touch_key(key)
                        break
