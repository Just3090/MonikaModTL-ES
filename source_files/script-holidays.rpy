...
label mas_o31_cleanup:
    ...
    call mas_transition_from_emptydesk ("monika 1hua") from _call_mas_transition_from_emptydesk_5
...
label greeting_o31_marisa:
    ...
    if store.mas_o31_event.cg_decoded:
        ...
        call spaceroom (hide_monika=True, scene_change=True) from _call_spaceroom_11
    else:



        call spaceroom (dissolve_all=True, scene_change=True, force_exp='monika 1eua_static') from _call_spaceroom_12
...
label greeting_o31_rin:
    ...
    call spaceroom (hide_monika=True, scene_change=True) from _call_spaceroom_13
    ...
    if store.mas_o31_event.cg_decoded:
        ...
        scene black
        pause 1.0
        call spaceroom (scene_change=True, dissolve_all=True, force_exp='monika 1hksdlb_static') from _call_spaceroom_14
...
label greeting_o31_orcaramelo_hatsune_miku:
    if not persistent._mas_o31_relaunch:
        call spaceroom (hide_monika=True, scene_change=True, dissolve_all=True) from _call_spaceroom_15
        ...
        call mas_transition_from_emptydesk ("monika 3hub") from _call_mas_transition_from_emptydesk_7
    else:
...
label greeting_o31_orcaramelo_sakuya_izayoi:
    call spaceroom (hide_monika=True, scene_change=True, dissolve_all=True) from _call_spaceroom_17
    ...
    call mas_transition_from_emptydesk ("monika 3hub") from _call_mas_transition_from_emptydesk_8
    ...
    call greeting_o31_deco from _call_greeting_o31_deco_3
...
label greeting_o31_briaryoung_shuchiin_academy_uniform:
    call spaceroom (hide_monika=True, scene_change=True, dissolve_all=True) from _call_spaceroom_18
    ...
    call mas_transition_from_emptydesk ("monika 2hub") from _call_mas_transition_from_emptydesk_9
    ...
    m 3hua "Ehehe~"
    call greeting_o31_deco from _call_greeting_o31_deco_4
...
label greeting_o31_hatana_2b:
    call spaceroom (hide_monika=True, scene_change=True, dissolve_all=True) from _call_spaceroom_19
    ...
    call mas_transition_from_emptydesk ("monika 3hub") from _call_mas_transition_from_emptydesk_10
    ...
    call greeting_o31_deco from _call_greeting_o31_deco_5
    call greeting_o31_cleanup from _call_greeting_o31_cleanup_5
    return
...
label greeting_o31_generic:
    call spaceroom (scene_change=True, dissolve_all=True) from _call_spaceroom_20
    ...
    call greeting_o31_deco from _call_greeting_o31_deco_6
    ...
    call greeting_o31_cleanup from _call_greeting_o31_cleanup_6
    return
...
label greeting_o31_cleanup(skip_zoom=False):
    window hide
    if not skip_zoom:
        call monika_zoom_transition (mas_temp_zoom_level, 1.0) from _call_monika_zoom_transition
    window auto
...
label greeting_o31_lingerie:
    ...
    call mas_o31_lingerie_end from _call_mas_o31_lingerie_end
    call greeting_o31_cleanup (skip_zoom=True) from _call_greeting_o31_cleanup_7
    return
...
label mas_o31_lingerie:
    ...
    call mas_o31_lingerie_end from _call_mas_o31_lingerie_end_1
    return
...
label mas_o31_lingerie_end:
    ...
    call spaceroom (scene_change=True, dissolve_all=True, force_exp='monika 2tfu') from _call_spaceroom_21
    ...
    call monika_zoom_transition (mas_temp_zoom_level, 1.0) from _call_monika_zoom_transition_1
    python:
...
label bye_trick_or_treat_wait_wait:
    ...
    menu:
        ...
        "You're right, it's too early." if too_early_to_go:
            ...
            call mas_transition_from_emptydesk (exp="monika 3hub") from _call_mas_transition_from_emptydesk_11
        ...
        "You're right, it's too late." if too_late_to_go:
            ...
            if persistent._mas_o31_tt_count:
                call mas_transition_from_emptydesk (exp="monika 1hua") from _call_mas_transition_from_emptydesk_12
                m 1hub "Ahaha~"
                m "I told you."
                m 1eua "We'll have to wait until next year to go again."
            else:

                call mas_transition_from_emptydesk (exp="monika 2dkc") from _call_mas_transition_from_emptydesk_13
                m 2dkc "..."
                m 2ekc "Alright, [player]."
                m "It sucks that we couldn't go trick or treating this year."
                m 4eka "Let's just make sure we can next time, okay?"
        ...
        "Actually, I can't take you right now.":
            ...
            call mas_transition_from_emptydesk (exp="monika 1euc") from _call_mas_transition_from_emptydesk_14
...
label bye_trick_or_treat_rtg:
    ...
    if _return:
        call mas_transition_from_emptydesk (exp="monika 1hub") from _call_mas_transition_from_emptydesk_15
    ...
    call mas_transition_from_emptydesk (exp="monika 1ekc") from _call_mas_transition_from_emptydesk_16
    $ persistent._mas_o31_tt_count -= 1
    m 1ekc "Oh no..."
...
label greeting_trick_or_treat_back:
    ...
    if persistent._mas_player_bday_in_player_bday_mode and not mas_isplayer_bday():
        ...
        call mas_o31_ret_home_cleanup (time_out, ret_tt_long) from _call_mas_o31_ret_home_cleanup_1
    return
...
label mas_o31_ret_home_cleanup(time_out=None, ret_tt_long=False):
    ...
    call mas_o31_cleanup from _call_mas_o31_cleanup
...
label mas_d25_monika_holiday_intro_deco:
    ...
    call spaceroom (scene_change=True, dissolve_all=True, bg_change_info=change_info) from _call_spaceroom_22
...
label mas_d25_spent_time_monika:
    ...
    if mas_isMoniEnamored(higher=True):
        ...
        call mas_showpoem (poem=mas_poems.getRandomPoem("d25"), background_action_label="mas_d25_poem_mistletoe") from _call_mas_showpoem
        ...
        menu:
            "I love you, [m_name].":
                ...
                m 6ekbfa "[player]...I...I..."
                call monika_kissing_motion (hide_ui=False) from _call_monika_kissing_motion
...
label mas_d25_monika_christmas_eve:
    ...
    if (
        ...
        if mas_hasUnlockedClothesWithExprop("lingerie"):
            ...
            m 5ekbfa "..."
            show monika 1ekbfa zorder MAS_MONIKA_Z at t11 with dissolve_monika
            call mas_lingerie_intro (holiday_str="on Christmas Eve", lingerie_choice=mas_clothes_santa_lingerie) from _call_mas_lingerie_intro_1
            m 1ekbfa "Just know that I love you very, very much, [player]~"
            $ mas_ILY()
    return
...
label mas_nye_monika_nye_dress_intro:
    ...
    if mas_SELisUnlocked(mas_clothes_dress_newyears):
        ...
        call mas_clothes_change (mas_clothes_dress_newyears, outfit_mode=True) from _call_mas_clothes_change_2
        ...
        call mas_clothes_change (mas_clothes_dress_newyears, outfit_mode=True, unlock=True) from _call_mas_clothes_change_3
...
label mas_player_bday_opendoor:
    ...
    call spaceroom (hide_monika=True, scene_change=True, dissolve_all=True, show_emptydesk=False) from _call_spaceroom_23
    $ mas_disable_quit()
    if mas_isMonikaBirthday():
        $ your = _("our")
    else:
...
label mas_player_bday_surprise:
    $ persistent._mas_player_bday_decor = True
    call spaceroom (scene_change=True, dissolve_all=True, force_exp='monika 4hub_static') from _call_spaceroom_24
    m 4hub "Surprise!"
...
label mas_player_bday_opendoor_listened:
    ...
    $ persistent._mas_player_bday_opened_door = True
    $ persistent._mas_player_bday_decor = True
    call spaceroom (hide_monika=True, scene_change=True, show_emptydesk=False) from _call_spaceroom_25
    $ mas_disable_quit()
    if mas_isMonikaBirthday():
        $ your = _("our")
    else:
...
label mas_player_bday_card:
    ...
    if mas_isMoniEnamored(higher=True):
        if persistent._mas_first_kiss is None:
            ...
            call monika_kissing_motion (duration=0.5, initial_exp="6hkbfa", fade_duration=0.5) from _call_monika_kissing_motion_4
            if mas_isMonikaBirthday():
...
label mas_monika_gets_cake:
    ...
    call mas_transition_from_emptydesk ("monika 6esa") from _call_mas_transition_from_emptydesk_17
...
label mas_player_bday_other_holiday:
    if mas_isO31():
        $ holiday_var = _("Halloween")
    elif mas_isD25():
        $ holiday_var = _("Christmas")
    elif mas_isF14():
        $ holiday_var = _("Valentine's Day")
    m 3euc "Hey, [player]..."
...
label return_home_post_player_bday:
    ...
    if not (mas_isMonikaBirthday() and mas_confirmedParty()):
        ...
        if not persistent._mas_player_bday_decor and not mas_isO31() and persistent._mas_o31_in_o31_mode:
            call mas_o31_ret_home_cleanup (time_out, ret_tt_long=False) from _call_mas_o31_ret_home_cleanup_2
...
label mas_f14_monika_valentines_intro:
    ...
    if not persistent._mas_f14_in_f14_mode or lingerie_eligible:
        ...
        if lingerie_eligible and not mas_hasUnlockedClothesWithExprop("lingerie"):
            call mas_lingerie_intro (holiday_str="on Valentine's Day", lingerie_choice=mas_clothes_vday_lingerie) from _call_mas_lingerie_intro_2
            ...
            if lingerie_eligible:
                call mas_clothes_change (outfit=mas_clothes_vday_lingerie, outfit_mode=True, exp="monika 2rkbsu", restore_zoom=False, unlock=True) from _call_mas_clothes_change_4
                pause 2.0
                show monika 2ekbsu
                pause 2.0
                show monika 2tkbsu
                ...
                call mas_clothes_change (mas_clothes_blackpink_dress, unlock=True, outfit_mode=True) from _call_mas_clothes_change_5
                m 2eua "Well...{w=0.3}what do you think?"
                call mas_f14_intro_blackpink_dress from _call_mas_f14_intro_blackpink_dress
            else:


                call mas_clothes_change (mas_clothes_sundress_white, unlock=True, outfit_mode=True) from _call_mas_clothes_change_6
            ...
            if (
                ...
                call mas_clothes_change (mas_clothes_sundress_white, unlock=True, outfit_mode=True) from _call_mas_clothes_change_7
                ...
                if monika_chr.clothes != mas_clothes_sundress_white:
                    ...
                    menu:
                        ...
                        "Yes.":
                            m 3hub "Okay!"
                            m 3eua "I'll be right back."
                            call mas_clothes_change (mas_clothes_sundress_white, unlock=True, outfit_mode=True) from _call_mas_clothes_change_8
                            m 2hub "There we go!"
        ...
        if not has_sundress:
            ...
            call mas_f14_intro_blackpink_dress from _call_mas_f14_intro_blackpink_dress_1
        else:
...
label mas_f14_monika_spent_time_with:
    ...
    if not mas_isBelowZero():
        ...
        if not mas_poem_vday_1.is_seen():
            call mas_showpoem (mas_poem_vday_1) from _call_mas_showpoem_2
            m "I really mean that, [player]..."
            m 3ekbsa "In you I found everything I could ever hope for~"

        elif not mas_poem_vday_2.is_seen():
            call mas_showpoem (mas_poem_vday_2) from _call_mas_showpoem_3
...
label mas_f14_first_kiss:
    ...
    menu:
        "I love you, [m_name].":
            ...
            show monika 6ekbfa zorder MAS_MONIKA_Z at t11 with dissolve_monika
            m 6ekbfa "[player]...I...I..."
            call monika_kissing_motion (hide_ui=False) from _call_monika_kissing_motion_6
            show monika 6ekbfa zorder MAS_MONIKA_Z at t11 with dissolve_monika
            m 6ekbfa "...I love you too~"
            m 6dkbfa "..."
            m "That was everything I had always dreamt it would be~"
...
label greeting_returned_home_f14:
    ...
    if persistent._mas_player_bday_in_player_bday_mode and not mas_isplayer_bday():
        call return_home_post_player_bday from _call_return_home_post_player_bday_13
...
label mas_bday_spent_time_with_wrapup:
    ...
    if mas_isMoniEnamored(higher=True):
        ...
        call monika_kissing_motion from _call_monika_kissing_motion_7
...
label mas_HideCake(cake_type, reset_zoom=True):
    ...
    with dissolve
    $ renpy.pause(3.0, hard=True)
    call mas_transition_from_emptydesk ("monika 6esa") from _call_mas_transition_from_emptydesk_19
    $ renpy.pause(1.0, hard=True)
    if reset_zoom:
        call monika_zoom_transition (mas_temp_zoom_level, 1.0) from _call_monika_zoom_transition_2
    return
