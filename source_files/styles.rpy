...
init python:
    style.console_text_console = Style(style.default)
    style.console_text_console.font = "DejaVuSans.ttf"
    style.console_text_console.size = 20
    style.console_text_console.color = "#FFFFFF"
    style.console_text_console.outlines = [(1, "#000000", 0, 0)]
...
init python:
    ...
    def mas_swapStyle(base_name, dark_name, morning_flag):
        if base_name.endswith(mas_ui.dark_suffix):
            return
        ...
        if morning_flag:
            ...
            if stashed_style is getattr(style, base_name):
                print("WARNING: Attempt to assign style '%s' to itself (morning_flag)" % base_name)
            setattr(style, base_name, mas_ui.style_stash[base_name])
        else:
            if hasattr(style, dark_name):
                dark_style = getattr(style, dark_name)
                if dark_style is getattr(style, base_name):
                    print("WARNING: Attempt to assign style '%s' to itself (dark_flag)" % base_name)
