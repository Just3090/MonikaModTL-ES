...
label introduction:
    ...
    if moni_exist():
        ...
        call mas_wx_cmd ("import os", local_ctx, w_wait=1.0) from _call_mas_wx_cmd
        call mas_wx_cmd ("os.remove(os.path.normcase(basedir+'/characters/monika.chr'))", local_ctx, w_wait=1.0, x_wait=1.0) from _call_mas_wx_cmd_1
...
label chara_monika_scare:
    ...
    show screen mas_py_console_teaching
    pause 1.0
    call mas_wx_cmd ("subprocess.call('" + str(bad_cmd) + "')", w_wait=3.0) from _call_mas_wx_cmd_2
    $ renpy.pause(2.0, hard=True)
    call mas_w_cmd (_("bye!")) from _call_mas_w_cmd
    pause 1.0
...
label ch30_reload_continuous_dlg:
    ...
    python:
        reload_quip_good = [
            ...
            _("...Oh... It's over and I'm back with you, [mas_get_player_nickname()]. That...that was awful.").format(""),
