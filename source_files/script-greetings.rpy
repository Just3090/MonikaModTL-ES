...
label monikaroom_greeting_ear_narration:
    ...
    $ mas_startupWeather()
    call spaceroom (dissolve_all=True, scene_change=True) from _call_spaceroom
...
label monikaroom_greeting_ear_renpy_docs:
    ...
    if mas_isMoniUpset():
        ...
        call monikaroom_greeting_ear_prog_upset from _call_monikaroom_greeting_ear_prog_upset_3

    elif mas_isMoniDis():
        m "...I {i}have{/i} to figure this out."
        call monikaroom_greeting_ear_prog_dis from _call_monikaroom_greeting_ear_prog_dis_3
...
label monikaroom_greeting_opendoor_locked:
    ...
    $ mas_startupWeather()
    call spaceroom (scene_change=True) from _call_spaceroom_1
...
label monikaroom_greeting_opendoor_seen_partone:
    ...
    call spaceroom (start_bg="bedroom", hide_monika=True, scene_change=True, dissolve_all=True, show_emptydesk=False, hide_calendar=True) from _call_spaceroom_2
    pause 0.2
...
label monikaroom_greeting_opendoor_post2:
    ...
    $ mas_startupWeather()
    call spaceroom (hide_monika=True, scene_change=True, show_emptydesk=False) from _call_spaceroom_3
...
label monikaroom_greeting_opendoor:
    ...
    call spaceroom (start_bg="bedroom", hide_monika=True, dissolve_all=True, show_emptydesk=False, scene_change=True, hide_calendar=True) from _call_spaceroom_4
...
label monikaroom_greeting_knock:
    ...
    menu:
        "It's me.":
            ...
            $ mas_startupWeather()
            call spaceroom (hide_monika=True, dissolve_all=True, scene_change=True, show_emptydesk=False) from _call_spaceroom_5
    jump monikaroom_greeting_post
...
label greeting_ourreality:
    ...
    if persistent._mas_current_background != "spaceroom":
        ...
        call mas_background_change (mas_background_def, skip_leadin=True, skip_outro=True, set_persistent=True) from _call_mas_background_change
        m 1eua "Here we are!"
    ...
    call mas_islands (fade_out=False, drop_shields=False, enable_interaction=False) from _call_mas_islands
    ...
    call mas_islands (fade_in=False, raise_shields=False, enable_interaction=False, force_exp="monika 1lsc") from _call_mas_islands_1
    ...
    call mas_islands (force_exp="monika 1eua") from _call_mas_islands_2
    return
...
label greeting_returned_home_cleanup:
    ...
    if not need_to_reset_bday_vars and not mas_isO31() and persistent._mas_o31_in_o31_mode:
        call mas_o31_ret_home_cleanup (time_out) from _call_mas_o31_ret_home_cleanup
...
label ch30_reload_delegate:
    ...
    if persistent.monika_reload >= 4:
        ...
        call expression reload_label from _call_expression_1
...
label greeting_code_help:
    ...
    pause 0.1
    hide noise
    call spaceroom (dissolve_all=True, scene_change=True, force_exp='monika 2wud_static') from _call_spaceroom_7
...
label greeting_after_bath:
    ...
    call spaceroom (hide_monika=True, dissolve_all=True, scene_change=True, show_emptydesk=True) from _call_spaceroom_8

    $ renpy.pause(random.randint(5, 15), hard=True)
    call mas_transition_from_emptydesk ("monika 1huu") from _call_mas_transition_from_emptydesk
    $ renpy.pause(2.0)
    $ quick_menu = True
...
label mas_after_bath_cleanup:
    ...
    call mas_transition_from_emptydesk ("monika 3hub") from _call_mas_transition_from_emptydesk_1
    window auto
...
label greeting_found_nou_shirt:
    ...
    call spaceroom (hide_monika=True, dissolve_all=True, scene_change=True, show_emptydesk=True) from _call_spaceroom_9
    pause 2.5
...
label greeting_found_nou_shirt.menu_skip:
    hide screen mas_background_timed_jump
    call mas_transition_from_emptydesk ("monika 4sub") from _call_mas_transition_from_emptydesk_2
    m "But I love it~"
...
label greeting_found_nou_shirt.menu_choice_secret:
    if mas_isMoniEnamored(higher=True):
        call mas_transition_from_emptydesk ("monika 2tublu") from _call_mas_transition_from_emptydesk_3
        m "{cps=*1.5}You don't peek there {i}often{/i}, do you?~{/cps}{w=0.1}{nw}"
        $ _history_list.pop()
        m 2lusdla "Anyway... {w=0.3}{nw}"
    else:

        call mas_transition_from_emptydesk ("monika 2rtblsdlu") from _call_mas_transition_from_emptydesk_4
