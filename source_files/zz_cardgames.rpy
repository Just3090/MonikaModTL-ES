...
init 5 python in mas_nou:
    ...
    class NOU(object):
        ...
        def _load_sfx(cls):
            ...
            """
            nou_ma_dir = "mod_assets/games/nou/sfx/"
            # Usar renpy.list_files para compatibilidad con Android
            try:
                nou_sfx = [f for f in renpy.list_files() if f.startswith(nou_ma_dir) and f.endswith(cls.SFX_EXT)]
            except Exception:
                nou_sfx = []
            ...
            for f in nou_sfx:
                # f es la ruta completa, por ejemplo: 'mod_assets/games/nou/sfx/shuffle_1.mp3'
                filename = f.split("/")[-1]
                if not filename.endswith(cls.SFX_EXT):
                    continue

                name, undscr, rest = filename.partition("_")
                sfx_list = name_to_sfx_list_map.get(name, None)
                if sfx_list is None:
                    continue

                sfx_list.append(f.replace("\\", "/"))
        ...
        def _select_help(self):
            ...
            if card.type == "number":
                dlg_line_list.append(
                    _("You need to play a{} '{}' or any {} card.").format(
                ...
                if player.should_skip_turn:
                    ...
                    if not player.drew_card:
                        if not get_house_rule("reflect_chaos"):
                            if card.type == "action":
                                ...
                                card_for_reflect = _("Draw Two")
                                color_for_reflect = card.color
                        
                        else:
                            if card.type == "action":
                                if card.label == "Draw Two":
                                    card_for_reflect = _("Draw Two{/i} or {i}Draw Four")
                        ...
                        dlg_line_list.append(
                            _(" If you have a {}{}{{i}}{}{{/i}}, you could {{i}}try{{/i}} to reflect {} card.").format(
                                color_for_reflect,
                    ...
                    if card.type == "action":
                        dlg_line_list.append(
                            _("You need to play a {{i}}{}{{/i}} or any {} card.").format(
                                card.label,
                                card.color
                            )
                        )
                        ...
                        if card.color is None:
                            ...
                            dlg_line_list.append(
                                _("You need to play any {} card.").format(card.color)
                            )
    ...
    class _NOUPlayerAI(_NOUPlayer):
        ...
        def _handle_nou_logic(self, current_reaction):
            ...
            if (
                ...
                if has_yelled_nou:
                    remind_quip = _("...And speaking of NOU...{w=0.5}") + remind_quip
...
label monika_change_nou_house_rules:
    ...
    label monika_change_nou_house_rules.menu_loop:
        ...
        if not _return:
            ...
            call monika_change_nou_house_rules.no_change from _call_monika_change_nou_house_rules_no_change
            ...
            m 1eub "Alright!"
            call monika_change_nou_house_rules.change_points_to_win_loop from _call_monika_change_nou_house_rules_change_points_to_win_loop

        elif _return == "starting_cards":
            m 1eub "Alright!"
            call monika_change_nou_house_rules.change_starting_cards_loop from _call_monika_change_nou_house_rules_change_starting_cards_loop
...
label monika_change_nou_house_rules.change_points_to_win_loop:
    ...
    while not ready:
        ...
        $ points_cap = store.mas_utils.tryparseint(
            renpy.input(
                _("How many points would you like it to be?"),
                allow=numbers_only,
                length=4
            ).strip("\t\n\r"),
            200
...
label monika_change_nou_house_rules.change_starting_cards_loop:
    ...
    while not ready:
        ...
        $ starting_cards = store.mas_utils.tryparseint(
            renpy.input(
                _("How many cards would you like to start the game with?"),
                allow=numbers_only,
                length=2
            ).strip("\t\n\r"),
            7
...
label mas_nou_game_end:
    ...
    call spaceroom (scene_change=True, force_exp="monika 1eua") from _call_spaceroom_28
    ...
    if store.mas_nou.winner == "Player":
        call mas_nou_reaction_player_wins_round from _call_mas_nou_reaction_player_wins_round
        ...
        if (
            ...
            call mas_nou_reaction_player_wins_game from _call_mas_nou_reaction_player_wins_game
        ...
        call mas_nou_reaction_monika_wins_round from _call_mas_nou_reaction_monika_wins_round
        ...
        if (
            ...
            call mas_nou_reaction_monika_wins_game from _call_mas_nou_reaction_monika_wins_game
        ...
        call mas_nou_reaction_player_surrenders from _call_mas_nou_reaction_player_surrenders
...
label mas_nou_game_end_change_rules_and_continue:
    call monika_change_nou_house_rules.pre_menu (from_game=True) from _call_monika_change_nou_house_rules_pre_menu
...
init -10 python in mas_cardgames:
    ...
    default_background_id = "spaceroom"
    ...
    def _m1_zz_cardgames__scanDeskSprites():
        ...
        file_list = store.MASDockingStation(GAME_DIR_PATH + DESK_SPRITES_PATH).getPackageList()
        ...
        # just6889: esto asegura que si hay submods, se use el predeterminado para evitar errores en codigo
        if default_background_id not in DESK_SPRITES_MAP:
            filename = sprites_map.get(default_background_id, fb)
            if filename is not None:
                DESK_SPRITES_MAP[default_background_id] = MASFilterSwitch(DESK_SPRITES_PATH + filename)
    ...
    class DeskSpriteSwitch(renpy.display.core.Displayable):
        ...
        def visit(self):
            ...
            if hasattr(store.persistent._mas_current_background, 'background_id'):
                background_id = store.persistent._mas_current_background.background_id
            else:
                background_id = store.persistent._mas_current_background

            if background_id not in DESK_SPRITES_MAP:
                background_id = default_background_id

            return [DESK_SPRITES_MAP[background_id]]
