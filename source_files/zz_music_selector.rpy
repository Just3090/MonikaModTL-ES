...
init -1 python in songs:
    ...
    def _m1_zz_music_selector__scanCustomBGM(music_list):
        """
        Escanea la carpeta custom_bgm usando renpy.list_files y agrega los .mp3 al menú de música.
        ...
        for f in files:
            if f.startswith(prefix) and f.lower().endswith(".mp3"):
                ...
                # Mostrar el nombre sin extensión como display name
                disp_name = filename[:-4] if filename.lower().endswith(".mp3") else filename
                music_list.append((
